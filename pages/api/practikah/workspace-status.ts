/**
 * BFF route: GET /api/practikah/workspace-status
 *
 * Forwards the authenticated physician's workspace-status request to the FastAPI
 * /practikah/workspace/status endpoint, forwarding the NextAuth HS256 JWT as
 * Authorization: Bearer per Phase 11 D-04.
 *
 * Per D-04: browser never touches FastAPI directly. This BFF route reads the
 * httpOnly NextAuth session cookie via getToken({ raw: true }) and forwards it
 * as a Bearer token. FastAPI's verified_physician dependency re-verifies the
 * HS256 signature and asserts verification_status='verified' (D-05, D-06).
 *
...
export default async function handler(
  ...
  try {
    const upstream = await fetch(`${FASTAPI_URL}/practikah/workspace/status`, {
      method: 'GET',
      headers: {
    ...
    console.error('Exception in workspace-status BFF:', err);
